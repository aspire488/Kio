...
| CAP.OPS.002 | CURRENT | CAP.OPS.001 | Repository, Live Telegram Validation | Diagnostic attribution: per-volume storage analysis (pressure per drive — "C: getting full, D: plenty of space" — never one global number), bounded storage contributor attribution (depth/time-limited scan, honest when unidentifiable), application-aware RAM/CPU consumer grouping (never cross-merged, disappeared processes never fabricated), structured conditions (severity/evidence/contributors/observed_at — proactive foundation), correlated "what's wrong" and "why is my computer slow" (never contradicts verified evidence), natural KIO voice (`mini_kio/core/operational_health.py`); pre-Slice-9 refinement (verified 2026-08-09). |
| CAP.CTX.001 | CURRENT | CAP.STATE.001 | Repository, Live Telegram Validation | System-level desktop state: "What's open?" / "What am I using?" / "What's active?" answered from deterministic observation — browser tabs + visible native windows (generic EnumWindows observation, no app special-cases; background processes never appear), hierarchical App—Window—Tab composition, OS-verified active window, KIO provenance, truthful empty/partial states, no LLM fabrication (`mini_kio/core/desktop_state.py`); pre-Slice-9 refinement (verified 2026-08-09). |
