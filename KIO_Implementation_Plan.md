...
| 9 — credential vault: lifecycle | **IMPLEMENTED + VERIFIED (2026-08-10)** | `credential_vault.validate/status/is_expired/refresh/revoke_by_target/reauthentication_required/needs_attention` + lifecycle prerequisite resolvers wired to the Slice 7 gate; user-facing `credential_management_result` (list/status/revoke/refresh/attention); 16 targeted tests (`tests/test_slice9_credential_lifecycle.py`) + live Telegram (`what credentials do I have` → truthful vault answer, no secrets) |
...
**Cross-cutting capability (pre-Slice-9 refinement, verified 2026-08-09): Diagnostic Attribution.** KIO now interprets system measurements instead of merely reading them. Per-volume storage analysis (C: 82% vs D: 0% reported per drive — pressure never collapsed into one global number); bounded storage contributor attribution (depth-2, 1.5s-budget scan — live-identified pagefile.sys ~21 GB / hiberfil.sys ~6 GB / Windows on C:); application-aware RAM/CPU consumer grouping (identical-binary grouping only, friendly names via the canonical desktop identity, disappeared processes never fabricated); structured diagnostic conditions (id/severity/current_value/threshold/evidence/contributors/observed_at) as the proactive-condition foundation; "what's wrong" correlates KIO + system conditions and never says "nothing wrong" while verified serious conditions exist; "why is my computer slow" ranks the strongest contributors; "high metric ≠ actual problem" (84% RAM without a verified contributor is not reported as a failure). Canonical owner: `mini_kio/core/operational_health.py` (`_storage_volumes`, `_storage_contributors`, `_top_consumers`, `_system_conditions`); new `_OPERATIONAL_ROUTES` family (resources / diagnostic / resources_ram / resources_cpu / resources_storage / which-drive-full / space-tail-anchored) with `kio_bot.py` `/resources` wiring. Cheap queries stay cheap — attribution only on diagnostic/elevated paths. Evidence: 45 targeted tests (`tests/test_diagnostic_attribution.py`) + 536-test regression gate (2 pre-existing gate2 failures baseline-identical) + live Telegram verification with values cross-checked against psutil. Not a roadmap slice — bounded capability refinement; no Slice status changed.

**Cross-cutting capability (pre-Slice-9 refinement, verified 2026-08-09): System-Level Desktop State.** "What's open?" / "What am I using?" / "What's active?" now report the user's actual desktop — browser tabs AND visible native windows — not merely the browser process. Root cause fixed: the previous composition read only browser tabs + KIO-tracked processes; native observation was absent, so externally launched apps (terminal, File Explorer, VS Code, arbitrary GUI apps) were invisible. Canonical owner: `mini_kio/core/desktop_state.py` (observation + hierarchical App—Window—Tab composition); generic read-only EnumWindows in `mini_kio/platform/window_activation.py` (`list_visible_windows`, `activate_window` — no app special-cases; background processes without a visible window never appear); `_exec_browser.list_tabs` delegates to the canonical composer; `_try_native_focus` gained a generic visible-window fallback that never targets browser hosts (no scope escalation); OS-verified foreground window is the sole authority for "(active)". Deterministic, side-effect-free, no LLM. Evidence: 46 targeted tests (`tests/test_desktop_state.py` + rewritten composition tests) + 504-test regression gate (2 pre-existing gate2 failures baseline-identical) + live Telegram verification: real desktop (Chrome tabs, Windows Terminal, File Explorer, Settings) reported with correct active marking; "Open Notepad" → appeared without any code patch; "Close it" → closed Notepad only. Not a roadmap slice — bounded capability refinement; no Slice status changed.

**Cross-cutting capability (pre-Slice-9 refinement, verified 2026-08-10): KIO-self Identity Routing + Typo-Tolerant NL + Anti-Fabrication.** Root mechanism fixes for the live probes ("How's Kio's health" → LLM; "Kio heath" → LLM; "KIO ok?" → offer-acceptance; LLM calling the user "Peter"): (1) canonical `_canonicalize_self_reference` in `pipeline/__init__.py` collapses contractions/casual spellings/trailing-KIO references/punctuation into the forms the deterministic route tables already understand — original text is preserved for providers; (2) bounded `_typo_fix_operational` corrects misspelled operational keywords only (ratio-gated, prefix/derived-form guarded — "resource usage" and "why are you unhealthy" are never corrupted); (3) KIO-as-self pre-check (`_detect_kio_self`) resolves KIO-prefixed self-state queries before name-strip can misroute them; (4) KIO-as-assistant vs KIO-as-external-entity boundary in `_check_identity` ("tell me about KIO Systems" → knowledge, "what is kio" → identity); (5) conversational identity family ("how are you, KIO?" → deterministic natural greeting, never LLM biography); (6) generic anti-fabrication guard `_sanitize_llm_name_address` strips invented user-name addresses from LLM output and the conversation prompt forbids name/identity invention; (7) "Want more detail?"-style robotic tails removed from the conversation policy. Live evidence (real Telegram round-trips, one message at a time): `Kio heath`, `How's Kio's health`, `KIO ok?`, `u good KIO?` → deterministic KIO health; `How are you, KIO?` → natural KIO greeting; `What is KIO?` → identity; `Tell me about KIO Systems` → knowledge. 15 new targeted tests (`tests/test_kio_identity_routing.py`); baseline-diff verified: 2 regressions introduced-and-fixed during the session, remaining suite failures baseline-identical. Not a roadmap slice — bounded capability refinement; no Slice status changed.

**Cross-cutting capability (pre-Slice-9 refinement, verified 2026-08-10): System Awareness — Installed-Application Inventory + Verified Web Synthesis + Battery Health + Short-Confirmation UX.** Root mechanism fixes for the live probes ("Open Stack Overflow" misrouted; "What's open?" missing elevated apps like Resolve; battery invisible in system health; "What apps do I have?" unanswerable; "is X connected?" falling to the LLM): (1) **installed-app inventory** — `app_inventory()` in `app_operator.py` enumerates generic Start Menu + registry + WindowsApps sources (no per-app list; 80+ apps live); new OPERATIONAL `app_inventory` route ("what apps do i have" / "what's installed") and `system_summary` route ("what's on my computer" — apps/drives/CPU/RAM/battery summary); (2) **verified multi-word web synthesis** — `_verified_brand_domain()` resolves recognizable multi-word entities ("stack overflow" → stackoverflow.com) with DNS + homepage-title verification that rejects parked/for-sale/domain-echo pages (davinciresolve.com, visualstudiocode.com); the `_web_fallback_url` char guard now allows spaces for multi-word input while still blocking injection chars; (3) **short-confirmation UX rule** (directive 2026-08-10 §22): successful opens — including web fallback — say "Opened X." (title-cased via `target_ref.display_target_name`), modality stays structured metadata so "close it" still resolves to the web target; disclosure of native-vs-web mechanics only on failure; (4) **empty-title window observation** — `window_activation.py` keeps real-app windows with unreadable (elevated, cross-integrity) titles instead of dropping them; `desktop_state.py` filters system shells generically, so elevated apps like DaVinci Resolve are no longer invisible in "What's open?"; (5) **battery/power health dimension** — `_system_metrics` gains battery_secsleft, `_fmt_battery_line` renders "Battery: N% (charging/on battery)" with remaining time when Windows exposes it, critical condition ≤6% / low ≤20% (charging excluded), desktop-without-battery reports honestly unavailable; (6) **credential-connected queries** — "is <provider> connected" routes to CREDENTIAL status for known providers (github/google/telegram/…), limited to the vault's provider surface so browser/component queries are never hijacked. Live Telegram evidence (one message at a time, real Windows): "open stack overflow" → "Opened Stack Overflow." (verified web target, shown in "what's open" with KIO provenance); "what apps do i have" → 46+ real installed apps; "what's on my computer" → 80+ apps / 2 drives / CPU/RAM/battery; "is github connected" → truthful "haven't connected GitHub oauth2 yet"; "kio health" → "KIO's healthy and running normally."; "open calculator"/"close calculator" → native launch+close (an app never mentioned in the directive); "what's open" → real desktop incl. elevated-visible windows. 283 targeted tests green in touched areas; full-suite delta vs baseline = 0 new failures (all remaining failures baseline-identical: stale response-format expectations, missing optional packages, port-9877 test pollution from the live bot). Not a roadmap slice — bounded capability refinement; no Slice status changed.
...
**STATUS — IMPLEMENTED + VERIFIED (2026-08-10).** Lifecycle delivered on the existing Slice 8 canonical (no new registry): `CredentialVault.validate()` returns the truthful state machine (`valid / missing / expired / revoked / unavailable / refreshable / refresh_failed`); `status()` / `is_expired()` / `revoke_by_target()` (idempotent, consent-gated) / `reauthentication_required()` / `needs_attention()`; `refresh()` performs an atomic keyring+metadata update through `OAuthFlowTemplate` wiring with truthful failure; the Execution Gate consumes state-qualified prerequisite identifiers (`credential_block_reason`) through the existing `_PREREQUISITE_RESOLVERS` registry — no parallel credential-checking path. User-facing surface is secret-free and interface-independent (`credential_management_result` in `credential_vault.py`; `CREDENTIAL` intent family in `pipeline/__init__.py`; `/credentials` PTB wiring). Security invariants unchanged: secrets never in logs/LLM context/responses/tests; consent recorded; revoke never exposes secret material. Evidence: 16 targeted tests (`tests/test_slice9_credential_lifecycle.py`) + Slice 8/7 suites + live Telegram round-trip (`what credentials do I have` → truthful "no connected accounts" vault answer; no provider invented).
