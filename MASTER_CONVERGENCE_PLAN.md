...
| `provider_base.py` | Stable, 31 lines. ABC for LLM providers. | Capability-based routing | Formerly duplicated by `providers/provider_base.py` (Protocol) | **Keep** — LLM failover chain is a distinct live subsystem |
...
| `providers/provider_base.py` (Protocol-based) | Built, 63 lines. Second provider interface — different from `mini_kio/llm/provider_base.py`. | Capability-based routing | Two provider philosophies sharing one name; zero importers (dead) | **Deleted 2026-08-09** (D-08) — dead package, zero importers; canonical contract is `core/provider_contract.py` `ExecutionProvider` |
...
| C-02a | Two `provider_base` interfaces: `mini_kio/llm/provider_base.py` (ABC — `generate()`, `health_check()`) vs. `providers/provider_base.py` (Protocol — `initialize()`, `shutdown()`, `health()`, `capabilities()`, `metadata()`, `execute()`). | Two different interface philosophies share the name "provider," serving genuinely different purposes (LLM text generation vs. generic capability execution). | Keep both, but rename the Protocol-based one to `CapabilityProvider` to disambiguate (D-08). | **Resolved 2026-08-09 — D-08** (dead Protocol package deleted; canonical contract = `core/provider_contract.py` `ExecutionProvider`) |
...
| D-08 | `providers/provider_base.py` (Protocol) | **Deleted 2026-08-09** | Zero importers in mini_kio/tests/root/config; previously deleted by `7eeb7c8` (July) and re-added as a dead snapshot by `c641220`. Deletion is the minimal convergence outcome (delete-over-create); the canonical execution-provider contract is `core/provider_contract.py` `ExecutionProvider` (ABC), wired through `core/provider_registry.py`. LLM `provider_base.py` kept — separate live failover subsystem. |
...
- **Providers:** Two LLM chains + two `provider_base` interfaces → one LLM chain (D-10), one canonical `ExecutionProvider` contract in `core/provider_contract.py` (D-08 resolved 2026-08-09 — dead Protocol package deleted) (Gate C-5).
