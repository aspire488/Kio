...
**DEMO SCENARIOS (live Telegram, 2026-08-09):** `Open ChatGPT` → "Opened ChatGPT in Chrome." · `Close it` → "Closed ChatGPT tab." (Chrome process count unchanged — tab scope, not browser kill) · `What is open?` → tabs resolved to canonical web-app identities (ChatGPT/Telegram/YouTube) + browser host line (`Browser: Chrome`), duplicates collapsed with counts — no raw titles/URLs · `Play Never Gonna Give You Up` → playback verified `paused=False` · `Pause it` → "Paused." · `What is playing?` → "Paused on Never Gonna Give You Up." · `Resume it` → "Resumed." · `Open ChatGPT and Telegram` → "Opened ChatGPT and Telegram." · `Hi` during media ≈1.7s while media continued · `Close ChatGPT tab` / `Close Telegram` → tab-scoped closes, Chrome alive.
