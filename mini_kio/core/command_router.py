...
def _dispatch_command(command: str) -> dict:
    ...
    # ── GO TO / NAVIGATE TO (NL → browser_goto or "open" alias) ──────────
    ...
    try:
        ...
        if lower.startswith("open "):
            ...
            if route_info["route_type"] == "browser_fallback":
                url = _normalize_web_target_to_url(target)
                if url:
                    result = execute_action("browser_goto", url)
                else:
        ...
        if lower.startswith("close "):
            ...
            # BrowserRuntime fallback: close tab if connector unavailable
            if _check_br_available():
        ...
        # ── STATIC ACTION TABLE ROUTING (before AI fallback) ──────────────────
        first_word = lower.split()[0] if lower.split() else ""
