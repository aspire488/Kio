...
# ── BrowserRuntime Configuration (Playwright engine) ──────────────
BROWSER_RUNTIME_ENABLED = os.getenv("BROWSER_RUNTIME_ENABLED", "true").lower() == "true"
BROWSER_RUNTIME_HEADLESS = os.getenv("BROWSER_RUNTIME_HEADLESS", "false").lower() == "true"
BROWSER_RUNTIME_BROWSER_TYPE = os.getenv("BROWSER_RUNTIME_BROWSER_TYPE", "chromium").lower()
BROWSER_RUNTIME_EXECUTABLE_PATH = os.getenv("BROWSER_RUNTIME_EXECUTABLE_PATH", "") or None
BROWSER_RUNTIME_DEFAULT_WORKSPACE = os.getenv("BROWSER_RUNTIME_DEFAULT_WORKSPACE", "default")

# ── MCP Runtime Configuration ─────────────────────────────────────
# MCP servers are configured via KIO_MCP_SERVERS JSON env var:
# [{"server_id": "...", "command": "...", "args": [...], "env": {...}}]
MCP_RUNTIME_ENABLED = os.getenv("MCP_RUNTIME_ENABLED", "false").lower() == "true"
_MCP_SERVERS_RAW = os.getenv("KIO_MCP_SERVERS", "")
MCP_SERVER_CONFIGS: list[dict] = []
if _MCP_SERVERS_RAW.strip():
    try:
        import json
        parsed = json.loads(_MCP_SERVERS_RAW)
        if isinstance(parsed, list):
            MCP_SERVER_CONFIGS = parsed
    except Exception:
        pass
